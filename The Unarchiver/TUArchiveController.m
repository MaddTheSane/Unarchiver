...
{
	if (foldermodeoverride >= 0)
		return foldermodeoverride;
	else
		return [[NSUserDefaults standardUserDefaults] integerForKey:UDKCreateFolderMode];
}

- (BOOL)copyArchiveDateToExtractedFolder
{
	if (copydateoverride >= 0)
		return copydateoverride != 0;
	else
		return [[NSUserDefaults standardUserDefaults] integerForKey:UDKModifyFolderDates] == 2;
}

- (void)setCopyArchiveDateToExtractedFolder:(BOOL)copydate
{
	copydateoverride = copydate;
}

- (BOOL)changeDateOfExtractedSingleItems
{
	if (changefilesoverride >= 0)
		return changefilesoverride != 0;
	else
		return [[NSUserDefaults standardUserDefaults] boolForKey:UDKModifyFileDates];
}

- (void)setChangeDateOfExtractedSingleItems:(BOOL)changefiles
{
	changefilesoverride = changefiles;
}

- (BOOL)deleteArchive
{
	if (deletearchiveoverride >= 0)
		return deletearchiveoverride != 0;
	else
...
{
	deletearchiveoverride = delete;
}

- (BOOL)openExtractedItem
{
	if (openextractedoverride >= 0)
		return openextractedoverride != 0;
	else
		return [[NSUserDefaults standardUserDefaults] boolForKey:UDKOpen];
}

- (void)setOpenExtractedItem:(BOOL)open
{
	openextractedoverride = open;
}
...
- (NSString *)filename
{
	if (!unarchiver)
		...
		return unarchiver.outerArchiveParser.filename;
}
...
{
	return unarchiver.archiveParser.caresAboutPasswordEncoding;
}
...
- (NSString *)currentArchiveName
{
	...
	if (!currfilename)
		currfilename = unarchiver.outerArchiveParser.currentFilename;
...
- (NSString *)simpleUnarchiver:(XADSimpleUnarchiver *)sender encodingNameForXADString:(id<XADString>)string
{
	...
	if (setencoding)
		return [XADString encodingNameForEncoding:setencoding];
	...
	if (selected_encoding)
		encoding = selected_encoding;
	else if (string.confidence * 100 >= threshold)
...
- (void)simpleUnarchiver:(XADSimpleUnarchiver *)sender
	extractionProgressForEntryWithDictionary:(NSDictionary *)dict
								fileProgress:(off_t)fileprogress
										  of:(off_t)filesize
							   totalProgress:(off_t)totalprogress
										  of:(off_t)totalsize
{
	double progress;
	if (totalsize)
		progress = (double)totalprogress / (double)totalsize;
	else
		progress = 1;
...
- (void)simpleUnarchiver:(XADSimpleUnarchiver *)sender
	estimatedExtractionProgressForEntryWithDictionary:(NSDictionary *)dict
										...
										totalProgress:(double)totalprogress
{
