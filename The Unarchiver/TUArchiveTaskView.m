...
- (BOOL)displayError:(NSString *)error ignoreAll:(BOOL *)ignoreall
{
	...
	if (res && ignoreall) {
		if (errorapplyallcheck.state == NSOnState)
			*ignoreall = YES;
		else
			*ignoreall = NO;
	}
...
- (NSStringEncoding)displayEncodingSelectorForXADString:(id<XADString>)string
{
	...
	if (res)
		return encodingpopup.selectedTag;
	else
		return 0;
}
...
- (NSString *)displayPasswordInputWithApplyToAllPointer:(BOOL *)applyall encodingPointer:(NSStringEncoding *)encoding
{
	...
	if (archive.caresAboutPasswordEncoding)
		*encoding = passwordpopup.selectedTag;
	else
		*encoding = 0;

	if (res && applyall) {
		if (passwordapplyallcheck.state == NSOnState)
			*applyall = YES;
		else
			*applyall = NO;
	}
...
- (void)setupEncodingViewForXADString:(id<XADString>)string
{
	...
	if (encoding) {
		...
		if (index >= 0)
			...
			[encodingpopup selectItemAtIndex:[encodingpopup indexOfItemWithTag:NSISOLatin1StringEncoding]];
	}
...
- (IBAction)selectEncoding:(id)sender
{
	...
	if ([namestring canDecodeWithEncoding:encoding])
		encodingfield.stringValue = [namestring stringWithEncoding:encoding];
	else
		encodingfield.stringValue = @""; // Can't happen, probably.
}
